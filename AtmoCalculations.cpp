...
pColorPacket CAtmoColorCalculator::AnalyzeHSV(tHSVColor *HSV_Img)
{
  ...
  int AtmoSetup_SatWinSize     = m_pAtmoConfig->getLiveView_SatWinSize();
   ...
   int AtmoSetup_Overlap     = m_pAtmoConfig->getLiveView_Overlap();
...
//  hsv_pixel.v = (unsigned char)new_value;
  ...
  */
  ...
  for (int zone = 0; zone < AtmoSetup_NumZones; zone++)
  {
    ...
    // convert back to rgb
	output_colors->zone[zone] = HSV2RGB(hsv_pixel,m_pAtmoConfig->getLiveView_Saturation());
  }
...
tHSVColor RGB2HSV(tRGBColor color)
{
 ...
 //   else
 //      // hsv.h = (171+15 + 43 * (rgb.r - rgb.g) / (rgbMax - rgbMin)) < 256 ? (171+15 + 43 * (rgb.r - rgb.g) / (rgbMax - rgbMin)) : ((171+15 + 43 * (rgb.r - rgb.g) / (rgbMax - rgbMin))-256);
	//	hsv.h = (171 + 43 * (rgb.r - rgb.g) / (rgbMax - rgbMin));
	//if ( hsv.h < 0 )     { hsv.h += h_MAX; }
 //   if ( hsv.h > h_MAX ) { hsv.h -= h_MAX; }
...
tRGBColor HSV2RGB(tHSVColor color, int saturation)
{
   /* tRGBColor rgb;
 ...
 float s = (float)color.s/(float)s_MAX*(float)((float)saturation/100.0f);
 if ( s > 1 ) { s = 1; }
 float v = (float)color.v/(float)v_MAX;
 //float v = (float)color.v/(float)v_MAX*(float)((float)saturation/100.0f);
 //if ( v > 1 ) { v = 1; }
 ...
 else
 {
   ...
   if (i == 0)
   {
     rgb.r = (int)((v*255.0)+0.5);//nein
     rgb.g = (int)((t*255.0)+0.5);
     rgb.b = (int)((p*255.0)+0.5);
   }
   else if (i == 1)
   {
     rgb.r = (int)((q*255.0)+0.5);//nein
     rgb.g = (int)((v*255.0)+0.5);
     rgb.b = (int)((p*255.0)+0.5);
   }
   else if (i == 2)
   {
     rgb.r = (int)((p*255.0)+0.5);//nein
     rgb.g = (int)((v*255.0)+0.5);
