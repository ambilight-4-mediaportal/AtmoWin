   ...
   for(int channel = 0; channel < channels ; channel++) {
       ...
       if((zoneIdx>=0) && (zoneIdx<data->numColors)) 
       {
          if(m_pAtmoConfig->isWhiteAdjPerChannel()) 
          {
             ...
             buffer[i_red++]   = (data->zone[zoneIdx].r * wr) / 256;
             buffer[i_green++] = (data->zone[zoneIdx].g * wg) / 256;
             buffer[i_blue++]  = (data->zone[zoneIdx].b * wb) / 256;

          } else {

             buffer[i_red++]   = data->zone[zoneIdx].r;
             buffer[i_green++] = data->zone[zoneIdx].g;
             buffer[i_blue++]  = data->zone[zoneIdx].b;
