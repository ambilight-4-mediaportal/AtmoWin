...
void CAtmoConfig::UpdateZoneDefinitionCount()
{
   ...
   {
      // okay zonen anzahl hat sich geaendert - wir muessen neu rechnen
      // und allokieren!
      if(m_ZoneDefinitions)
      {
        for(int zone=0; zone<m_AtmoZoneDefCount; zone++)
...
int CAtmoConfig::getArduComport(int i)
{
  if(i == 0)
...
void CAtmoConfig::setArduComport(int i, int nr)
{
  if(i == 0)
