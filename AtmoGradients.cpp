...
#define radio_height 20
       ...
       if(m_pConfig->getZonesTopCount() > 0) 
       {
            ...
            {
                sprintf( zonename, "Zone %d", zonenIdx );
       ...
       if( m_pConfig->getZoneSummary() )
       {
            ...
            sprintf( zonename, "Summenzone %d", zonenIdx );
            HWND handle = CreateWindow("BUTTON", zonename,
                             BS_AUTORADIOBUTTON  | BS_RIGHT | WS_VISIBLE | WS_TABSTOP | WS_CHILD, 
       ...
       char buffer[32];
       sprintf(buffer,"Edge weightning [%d]",m_edge_weight);
...
{ 
  ...
  if((hwnd == groupBox) && (uMsg == WM_DROPFILES) && (m_active_zone>=0))
  {
           ...
           if(azone)
           {
              ...
              switch(azone->LoadGradientFromBitmap( psz_filename ))
              {
                 case ATMO_LOAD_GRADIENT_OK:   
                      ...
                      {
                         sprintf( msg, "Datei '%s' konnte nicht überschrieben werden.", psz_filename_dest);
                         MessageBox(m_hDialog, msg ,"Fehler", MB_ICONERROR );
                      } else
                         MessageBox(m_hDialog, "Gradient geladen ok.","Info",MB_OK);
                      break;
                 case ATMO_LOAD_GRADIENT_FILENOTFOND:   
                      MessageBox(m_hDialog, "Datei nicht gefunden?","Fehler",MB_ICONERROR | MB_OK);
                      break;
                 case ATMO_LOAD_GRADIENT_FAILED_SIZE:   
                      MessageBox(m_hDialog, "Dateigröße / Auflösung stimmt nicht!","Fehler",MB_ICONERROR | MB_OK);
                      break;
                 case ATMO_LOAD_GRADIENT_FAILED_HEADER:   
                      MessageBox(m_hDialog, "Datei ist vermutlich keine BMP Datei!","Fehler",MB_ICONERROR | MB_OK);
                      break;
                 case ATMO_LOAD_GRADIENT_FAILED_FORMAT:   
                      MessageBox(m_hDialog, "Datei hat nicht die richtige Farbtiefe (8-bit, 24-bit)?","Fehler",MB_ICONERROR | MB_OK);
                      break;
              }
           }
           return(r); 
      ...
      MessageBox( m_hDialog, "Bitte nur BMP Dateien auf die Fläche ziehen!","Fehler",MB_ICONERROR | MB_OK);
  }
  return(r);
}
...
INT_PTR CALLBACK CAtmoGradients::GroupBoxProc(HWND hwnd,UINT uMsg, WPARAM wParam, LPARAM lParam) {
    CAtmoGradients *self; // und täglich grüßt delphi!
    int wmId, wmEvent;
    HWND hControl;
