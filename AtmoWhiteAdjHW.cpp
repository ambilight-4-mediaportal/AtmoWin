...
void CAtmoWhiteAdjHW::UpdateHardware() {
    ...
    pAtmoConnection->HardwareWhiteAdjust(m_Hardware_global_gamma, m_Hardware_global_contrast, m_Hardware_contrast_red, m_Hardware_contrast_green, m_Hardware_contrast_blue, m_Hardware_gamma_red, m_Hardware_gamma_green, m_Hardware_gamma_blue, ATMO_FALSE);
}
...
ATMO_BOOL CAtmoWhiteAdjHW::Execute(HINSTANCE hInst, HWND parent, CAtmoDynData *pAtmoDynData, ATMO_BOOL &saveLaterToHardware) {
       ...
       if(!pAtmoConnection || !pAtmoConnection->isOpen()) {
          MessageBox(parent,"Der Hardware-Weisabgleich ist nur mit einer aktiven Verbindung zur Hardware möglich.","Fehler",MB_ICONERROR | MB_OK);
          return ATMO_FALSE;
       }
